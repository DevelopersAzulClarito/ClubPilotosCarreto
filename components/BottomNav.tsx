...
const NavItem = React.memo<NavItemProps>(({ label, IconComp, isActive, onClick, isPrimary }) => {
    if (isPrimary) {
        return (
            <div className="relative flex flex-col items-center justify-center -top-5">
                <button
                    ...
                    className={`flex items-center justify-center w-14 h-14 rounded-full text-white shadow-xl transform transition-all duration-300 active:scale-95 ${
                        isActive ? 'bg-emerald-600 shadow-emerald-500/40 scale-105' : 'bg-[#e35212] shadow-orange-500/40'
                    }`}
                >
    ...
    return (
        <button onClick={onClick} className="relative flex flex-col items-center justify-center w-full h-full pt-2 pb-1 group">
            <div className={`absolute top-1.5 flex items-center justify-center w-14 h-8 rounded-full transition-all duration-300 ${isActive ? 'bg-orange-50 scale-100 opacity-100' : 'scale-50 opacity-0'}`}></div>
            <div className={`relative z-10 transition-all duration-300 ${isActive ? 'text-[#e35212] -translate-y-0.5' : 'text-gray-400 group-hover:text-gray-500'}`}>
                <IconComp className="w-6 h-6" />
            </div>
            <span className={`relative z-10 text-[10.5px] mt-1 transition-all duration-300 ${isActive ? 'font-bold text-[#e35212]' : 'font-medium text-gray-500'}`}>
                {label}
            </span>
        </button>
    );
...
const BottomNav = React.memo<BottomNavProps>(({ activeTab, setActiveTab }) => (
    <div
    ...
    >
        <div className="flex items-center justify-around h-[4.5rem] px-2 relative">
            <NavItem label="Inicio"  IconComp={IconHome}  isActive={activeTab === 'home'}    onClick={() => setActiveTab('home')} />
            <NavItem label="Tienda"  IconComp={IconStore} isActive={activeTab === 'store'}   onClick={() => setActiveTab('store')} />
            <NavItem label="Pase"    IconComp={IconQR}    isActive={activeTab === 'qr'}      onClick={() => setActiveTab('qr')} isPrimary />
            <NavItem label="Premios" IconComp={IconGift}  isActive={activeTab === 'prizes'}  onClick={() => setActiveTab('prizes')} />
            <NavItem label="Perfil"  IconComp={IconUser}  isActive={activeTab === 'profile'} onClick={() => setActiveTab('profile')} />
        </div>
    </div>
));
